...
// Token schema
const tokenSchema = new mongoose.Schema({ totalTokensUsed: Number });
const TokenCounter = mongoose.model('TokenCounter', tokenSchema);
...
// SerpAPI setup
if (!process.env.SERP_API_KEY) console.warn("⚠️ Missing SERP_API_KEY, live queries may fail");
...
app.post('/chat', async (req, res) => {
  ...
  try {
    ...
    // Update global token count in MongoDB
    let counter = await TokenCounter.findOne();
    if (!counter) counter = new TokenCounter({ totalTokensUsed: 0 });
    counter.totalTokensUsed += tokensUsed;
    await counter.save();

    res.json({ reply, tokensUsed, totalTokensUsed: counter.totalTokensUsed });

  } catch (err) {
    console.error(err.response?.data || err.message || err);
    res.status(500).json({ reply: "Error processing request", tokensUsed: 0, totalTokensUsed: 0 });
  }
});
